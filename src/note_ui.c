...
void create_note_window(Note *note) {
  if (note->window.handler == NULL)
    note->window.handler =
        newwin(window_size.h, window_size.w, note->window.position.y,
               note->window.position.x);
}
...
void note_window_display(const Note *note) {
  ...
  wbkgd(note->window.handler,
        COLOR_PAIR(note->focused ? COLOR_PAIR_NOTE_FOCUSED
  ...
  mvwprintw(note->window.handler, 0, 0, "#%ld", note->index);
