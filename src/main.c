...
int main(int argc, char **argv) {
  ...
  while (!quit) {
    ...
    if (count_notes(note_list_head) == 0) {
      show_information_dialog(
          "Your board is empty. Type 'c' to create your first note.");
    }
    ...
    case KEY_DC:
      if (selected_link &&
          show_confirmation_dialog("Do you really want to delete this note?")) {
  ...
  char *printable_doc = print_formatted ? cJSON_Print(updated_doc)
                                        : cJSON_PrintUnformatted(updated_doc);
